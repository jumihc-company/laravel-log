...
use Jmhc\Support\Utils\StorageUnitConversion;
use Monolog\Logger;
...
class StreamHandler extends \Monolog\Handler\StreamHandler
{
    ...
    public function __construct($stream, bool $debug = false, bool $withRequestInfo = false, string $maxSize = '0', int $maxFiles = 0, $level = Logger::DEBUG, bool $bubble = true, ?int $filePermission = null, bool $useLocking = false)
    {
        $this->debug = $debug;
     ...
     * @return array
     */
    protected function getFiles($fileInfo)
    {
        return glob(sprintf(
            '%s/%s.*%s',
            $fileInfo['dirname'],
     ...
     * @return array
     */
    protected function filesSort(array $files, string $extension, string $order = 'desc')
    {
